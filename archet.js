...
export class Root extends Component {
  constructor(targetId) {
    ...
    // Modern Mobile Height Fix (100dvh) with fallback
    this.css({ 
      width:"100%", 
    ...
    // Apply modern dynamic viewport height if supported
...
export class Crud extends Box {
  constructor(title, schema=[]) {
    ...
    this.onError = alert; // Overridable Error Handler
  ...
  renderList() {
    ...
    this.data.forEach((item, idx) => {
      ...
      // UPDATE: Added .data("role", "delete") for stable testing
      row.add(new Button("×", () => {
